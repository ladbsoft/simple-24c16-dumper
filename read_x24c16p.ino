...
const byte DEVADDR_FROM = 0x50;
const byte DEVADDR_TO = 0x57;
...
void loop() {
  if (Serial.available()) {
    ...
    //////////////////////////////////////////////////////////////////////////////
    if (incomingByte == 99) {
      for (byte addr = DEVADDR_FROM; addr <= DEVADDR_TO; addr++) {
        for (int i = 15; i <= 255; i += 16) {
          eeprom_write_page(addr, i, msgf, 16);
        }
    ...
    if (incomingByte == 115) {
      for (byte addr = DEVADDR_FROM; addr <= DEVADDR_TO; addr++) {
        for (int i = 0; i <= 255; i++) {
          byte b = eeprom_read_byte(addr, i);
          Serial.print(b, HEX);
          Serial.print(' ');
          if ((i + 1) % 16 == 0) {
            //Serial.print("  //");
